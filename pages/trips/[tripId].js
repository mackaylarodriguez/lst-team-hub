...
export default function TripPage() {
  ...
  async function dismissRequiredTripDocumentSlot(slot) {
    ...
    const demotedById = new Map(demotedUpdated.map((u) => [u.id, u]));
    setDocs((current) => {
      const filtered = (current || []).filter((e) => !idsToDelete.includes(e.id));
      const merged = filtered.map((e) => demotedById.get(e.id) || e);
      return prependDocWithoutDuplicates(merged, created);
    });
    setDocsError("");
  }
  ...
  async function handleDeleteRequiredSlotResource(slot) {
    ...
    if (
      ...
      !window.confirm(
        `Remove “${slot.title}” as a required card? Any PDFs or links already uploaded for this slot stay on the trip as extra documents (they are not deleted). You can restore the default empty card with the button below.`
      )
    ) {
      return;
    }
    try {
      ...
      showToast("Required card removed.");
    } catch (error) {
            ...
            {(() => {
              ...
              return (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div className="small" style={{ marginBottom: 10, opacity: 0.88 }}>
                    Auto-generated from your trip setup — budget tracking and project record journal.
                  </div>
