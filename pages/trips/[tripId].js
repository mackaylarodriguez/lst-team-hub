          ...
          {canViewTeamDashboard ? (
            <>
              ...
              <button
                className="btn"
                type="button"
                onClick={() => handleToggleDocVisibility(d, d.visibleToParticipants === false)}
              >
                {d.visibleToParticipants === false
  ...
  async function handleDeleteRequiredSlotResource(slot) {
    ...
    if (doc?.id && !doc?.isAutoGenerated) {
      ...
      if (slot.key === "smartsheet-budget") {
        const journalRow = (docs || []).find((d) => d.resourceKey === "project-record-journal");
        if (journalRow) await handleDeleteDoc(journalRow.id);
      }
      return;
    }
...
function parseDateSafe(dateStr) {
  ...
  const viewerRequiredDocumentSlots = useMemo(() => {
    if (canViewTeamDashboard) return effectiveRequiredDocumentSlots;
    return effectiveRequiredDocumentSlots.filter((slot) => {
      if (slot.key === "smartsheet-budget") {
        const res = slot.resource;
        if (!res) return true;
        return res.visibleToParticipants !== false;
      }
      return !hiddenRequiredDocumentKeys.has(slot.key);
    });
              ...
              {tripDocumentRows
                .filter(
                  (row) =>
                    row.kind !== "site" ||
                    canViewTeamDashboard ||
                    !hiddenRequiredDocumentKeys.has("site-info-link")
                )
                .map((row) => {
                  if (row.kind === "site") {
                    const siteSlotStub = {
