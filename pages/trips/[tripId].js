...
export default function TripPage() {
  ...
  useEffect(() => {
    ...
    async function loadHousingFromBudget() {
      try {
        const [linkUrl, pdfUrl] = await Promise.all([
          getTripHousingLinkForViewer(trip.id),
          getTripHousingPdfUrlForViewer(trip.id),
        ]);
        if (!cancelled) {
          setTripHousingLinkUrl(linkUrl);
          setTripHousingPdfUrl(pdfUrl);
        }
      } catch {
        if (!cancelled) {
