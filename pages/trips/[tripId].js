...
function isPersistedTripResourceDismissedEmpty(doc) {
  ...
  if (doc.visibleToParticipants !== false) return false;
  return !String(doc.link || "").trim() && !String(doc.pdfUrl || "").trim();
}
...
async function fetchTripHousingState(tripId) {
  ...
  try {
    ...
    return {
      ...
      tripHousingLinkUrl: primary?.link || "",
      tripHousingPdfUrl: primary?.pdfUrl || "",
    };
  } catch {
    return { tripHousingDocuments: [], tripHousingLinkUrl: "", tripHousingPdfUrl: "" };
  }
...
export default function TripPage() {
  ...
  /**
   * Remove a default required document slot for this trip: delete saved rows for that key,
   * then insert a hidden empty placeholder so the card stays out of the list until restored.
   */
  async function dismissRequiredTripDocumentSlot(slot) {
    if (!trip?.id || !slot?.key) return;
    const key = slot.key;
    const idsToDelete = [];
    for (const d of docs || []) {
    ...
    setDocs((current) => {
      ...
      return prependDocWithoutDuplicates(filtered, created);
    });
    setDocsError("");
  }
  ...
  async function handleDeleteRequiredSlotResource(slot) {
    ...
    if (
      ...
      !window.confirm(`Remove “${slot.title}” from this trip’s documents? You can restore default cards with the link below.`)
    ) {
      return;
    }
    try {
      await dismissRequiredTripDocumentSlot(slot);
      showToast("Document removed for this trip.");
    } catch (error) {
...
function parseDateSafe(dateStr) {
  ...
  const effectiveHousingLinkDoc = useMemo(() => {
    ...
    if (saved && !saved.isAutoGenerated && isPersistedTripResourceDismissedEmpty(saved)) {
      return null;
    }
    const rawLink = String(tripHousingLinkUrl || "").trim();
    const rawPdf = String(tripHousingPdfUrl || "").trim();
    ...
    // Trip Documents "Edit housing" writes trip_budgets; a legacy trip_resources row must not
    // hide those URLs. Prefer budget per field when that field is set on the budget row.
  ...
  const viewerRequiredDocumentSlots = useMemo(() => {
    return effectiveRequiredDocumentSlots.filter((slot) => {
      if (findDismissedPersistedTripResource(docs, slot.key)) return false;
      ...
      const persisted = (docs || []).find(
        (d) => String(d.resourceKey) === slot.key && !d.isAutoGenerated
      );
      return !!(
        persisted &&
        (String(persisted.link || "").trim() || String(persisted.pdfUrl || "").trim())
      );
    });
  }, [canViewTeamDashboard, docs, effectiveRequiredDocumentSlots, hiddenRequiredDocumentKeys]);
