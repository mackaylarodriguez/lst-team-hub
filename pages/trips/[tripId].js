...
export default function TripPage() {
  ...
  useEffect(() => {
    ...
    async function loadHousingFromBudget() {
      try {
        const list = await getTripHousingDocumentsForViewer(trip.id);
        if (!cancelled) {
          const docs = Array.isArray(list) ? list : [];
          setTripHousingDocuments(docs);
          const primary = docs[0];
          setTripHousingLinkUrl(primary?.link || "");
          setTripHousingPdfUrl(primary?.pdfUrl || "");
        }
      } catch {
        if (!cancelled) {
          setTripHousingDocuments([]);
          setTripHousingLinkUrl("");
          setTripHousingPdfUrl("");
        }
      }
    }
  ...
  /**
   * Delete a required-slot resource for this team.
   * - Saved docs are deleted outright.
   * - Auto-generated links are converted to a hidden override for this team.
   */
  async function handleDeleteRequiredSlotResource(slot) {
    ...
    await handleToggleRequiredSlotVisibility(slot, false);
  }
                ...
                >
                  ...
                  {(effectiveSiteInfoDoc?.link || effectiveSiteInfoDoc?.pdfUrl) ? (
                    <div className="row" style={{ marginTop: 10 }}>
                      ...
                      {canViewTeamDashboard && effectiveSiteInfoDoc?.isAutoGenerated ? (
                        <button
                          className="btn"
                          type="button"
                          style={siteLinkActionButtonStyle}
                          onClick={() =>
                            void handleDeleteRequiredSlotResource({
                              key: "site-info-link",
                              title: "Site Logistics",
                              category: "Site",
                              kind: "link",
                ...
                return (
                  ...
                  >
                              ...
                              <div className="small" style={{ marginTop: 4 }}>
                                {slot.key === "housing-accommodation-link"
                                  ? tripHousingDocuments.length > 1
                                    ? "Auto-added from Budget → Housing: main row plus additional slots for this team."
                                    : "Auto-added from Budget → Housing (link and/or PDF) for this team."
                    ...
                    <div className="row" style={{ marginTop: 10, flexWrap: "wrap", gap: 8 }}>
                      ...
                      {canViewTeamDashboard && !isEditing && available && isAutoGenerated ? (
                        <button
                          className="btn"
                          type="button"
