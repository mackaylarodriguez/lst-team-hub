...
import { resolveClassroomModuleSlotKey } from "@/lib/training";
import {
...
export function getPrototypeSectionsForClassroomModule(module) {
  const slot = resolveClassroomModuleSlotKey(module);
  const protoIdFromSlot = slot ? `proto-module-${slot}` : null;
  const protoId =
    (protoIdFromSlot && getPrototypeModuleById(protoIdFromSlot) ? protoIdFromSlot : null) ||
    findPrototypeModuleIdForTrainingTitle(module?.title);
  return getPrototypeModuleById(protoId)?.sections || [];
}

/** If a classroom module is already complete, treat every matching training section as complete. */
export function applyCompletedClassroomModulesToSectionState(
  sectionState = {},
  trainingState = {},
  modules = []
) {
  const next = { ...(sectionState || {}) };
  const newlyCompletedSectionIds = [];

  for (const module of modules || []) {
    if (!trainingState?.[String(module.id)]) continue;
    for (const section of getPrototypeSectionsForClassroomModule(module)) {
      if (next[section.id]) continue;
      next[section.id] = true;
      newlyCompletedSectionIds.push(section.id);
    }
  }

  return { sectionState: next, newlyCompletedSectionIds };
}
