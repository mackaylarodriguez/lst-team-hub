...
async function loadStaffOverviewBase() {
  ...
  const workerProfiles = (profilesResult.data || []).filter(
    (profile) => normalizeText(profile.role) === ROLE_WORKER
  );
      ...
      const assignmentDetails = userAssignments
        .map((assignment) => {
          ...
          return {
            ...
            taskPercent: percent(taskCompletedForTrip, tripTasks.length),
            trainingPercent: percent(trainingCompletedForTrip, tripModules.length),
  ...
  const tripMetricsById = Object.fromEntries(
    trips.map((trip) => {
      ...
      const workerCount = tripAssignments.length;
      const tripTasks = tripTasksByTripId.get(trip.id) || [];
      const tripModules = modulesByTripId.get(trip.id) || [];
