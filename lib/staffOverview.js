...
function getMetricUserIdsForTrip(tripId, visibleAssignments, rosterWorkerUserIdsByTrip) {
  const set = new Set();
  for (const a of visibleAssignments || []) {
    if (a.trip_id === tripId) set.add(a.user_id);
  }
  const roster = rosterWorkerUserIdsByTrip.get(tripId);
...
async function loadStaffOverviewBase() {
  ...
  for (const idSet of rosterWorkerUserIdsByTrip.values()) {
