...
/** Same headcount rules as the trip card “Workers” tile (excludes leaders not traveling). */
function isRosterMemberForWorkerCount(row) {
...
function countRosterWorkersByTripId(rows, tripIdKey = "trip_id") {
  ...
  for (const row of rows || []) {
    const id = row?.[tripIdKey];
    if (!id) continue;
    if (!isRosterMemberForWorkerCount(row)) continue;
    map.set(id, (map.get(id) || 0) + 1);
  }
  return map;
}

/** `user_task_progress.task_name` is usually the trip_tasks row id; older rows may store the task title. */
function isWorkerTaskComplete(completedTaskProgress, tripId, userId, task) {
  const tid = String(task?.id || "").trim();
  const title = String(task?.title || "").trim();
  if (tid && completedTaskProgress.get(`${tripId}:${userId}:${tid}`)) return true;
  if (title && completedTaskProgress.get(`${tripId}:${userId}:${title}`)) return true;
  return false;
}

function buildRosterWorkerProfileMaps(teamMemberRows, rosterProfileRows) {
  const profileIdByEmail = new Map();
  for (const p of rosterProfileRows || []) {
    const role = normalizeText(p.role);
    if (role && role !== ROLE_WORKER) continue;
    const em = normalizeText(p.email);
    if (!em || profileIdByEmail.has(em)) continue;
    profileIdByEmail.set(em, p.id);
  }

  const userIdsByTrip = new Map();
  for (const row of teamMemberRows || []) {
    if (!isRosterMemberForWorkerCount(row)) continue;
    const tripId = row.trip_id;
    if (!tripId) continue;
    const em = normalizeText(row.email);
    if (!em) continue;
    const uid = profileIdByEmail.get(em);
    if (!uid) continue;
    if (!userIdsByTrip.has(tripId)) userIdsByTrip.set(tripId, new Set());
    userIdsByTrip.get(tripId).add(uid);
  }

  return { profileIdByEmail, userIdsByTrip };
}

/**
 * Roster row → profile id for any role (staff, leader, worker). Used so trip card % includes
 * everyone on the team roster who has a profile, not only rows with role worker.
 */
function buildRosterAllRoleUserIdsByTrip(teamMemberRows, rosterProfileRows) {
  const profileIdByEmail = new Map();
  for (const p of rosterProfileRows || []) {
    const em = normalizeText(p.email);
    if (!em || profileIdByEmail.has(em)) continue;
  ...
  return userIdsByTrip;
}

/** Union of trip_assignments (any role) and roster-derived profile ids for aggregate task/training %. */
function getMetricUserIdsForTrip(tripId, tripAssignments, rosterUserIdsByTrip) {
  const set = new Set();
  for (const a of tripAssignments || []) {
    if (a.trip_id === tripId) set.add(a.user_id);
  }
  const roster = rosterUserIdsByTrip.get(tripId);
  if (roster) roster.forEach((id) => set.add(id));
  return [...set];
}
...
function determineReadiness({
  ...
  /** `null` = no tasks/modules configured yet for this slice (do not treat as 0% / “behind”). */
  const hasTasks = taskPercent !== null && taskPercent !== undefined;
  const hasTraining = trainingPercent !== null && trainingPercent !== undefined;
  const taskScore = hasTasks ? taskPercent : 100;
  const trainScore = hasTraining ? trainingPercent : 100;

  const allFundraisingReady =
    fundraisingAllPercent === null ||
    fundraisingAllPercent === 100;

  if (taskScore === 100 && trainScore === 100 && allFundraisingReady) {
    return "Ready";
  }

  if (
    (hasTasks && taskPercent < 50) ||
    (hasTraining && trainingPercent < 50) ||
    (fundraising2000Percent !== null && fundraising2000Percent < 100 && daysUntilNearestTrip !== null && daysUntilNearestTrip <= 45) ||
    (daysUntilNearestTrip !== null &&
      daysUntilNearestTrip <= 21 &&
      ((hasTasks && taskPercent < 80) || (hasTraining && trainingPercent < 80)))
  ) {
    return "Behind";
  }
...
async function loadStaffOverviewBase() {
  ...
  const rosterSafeRows = teamMembersError ? [] : teamMemberRows || [];
  const rosterCountByTripId = countRosterWorkersByTripId(rosterSafeRows);
  const assignmentCountByTripId = countByTripId(assignments);

  const rosterEmails = [
    ...new Set(
      rosterSafeRows
        .filter(isRosterMemberForWorkerCount)
        .map((row) => String(row.email || "").trim())
        .filter(Boolean)
    ),
  ];

  let rosterProfileRows = [];
  if (rosterEmails.length > 0) {
    const rosterProfilesResult = await supabase
      .from("profiles")
      .select("id, email, role, first_name, last_name")
      .in("email", rosterEmails);
    if (rosterProfilesResult.error) {
      console.error("Error loading roster profiles for staff overview", rosterProfilesResult.error);
    } else {
      rosterProfileRows = rosterProfilesResult.data || [];
    }
  }

  const { userIdsByTrip: rosterWorkerUserIdsByTrip } = buildRosterWorkerProfileMaps(
    rosterSafeRows,
    rosterProfileRows
  );
  const rosterAllRoleUserIdsByTrip = buildRosterAllRoleUserIdsByTrip(rosterSafeRows, rosterProfileRows);

  const assignmentUserIds = [...new Set((assignments || []).map((item) => item.user_id).filter(Boolean))];
  const allProgressUserIds = new Set(assignmentUserIds);
  for (const idSet of rosterAllRoleUserIdsByTrip.values()) {
    for (const uid of idSet) allProgressUserIds.add(uid);
  }
  const userIds = [...allProgressUserIds];
    ...
    .map(([userId, userAssignments]) => {
      ...
      const assignmentDetails = userAssignments
        .map((assignment) => {
          ...
          const tripModules = (modulesByTripId.get(trip.id) || []).filter(isClassroomModuleRow);
          const taskProgressByTaskId = new Map(
            tripTasks.map((task) => [
              task.id,
              isWorkerTaskComplete(completedTaskProgress, trip.id, userId, task),
            ])
          );

          const taskCompletedForTrip = tripTasks.filter((task) =>
            isWorkerTaskComplete(completedTaskProgress, trip.id, userId, task)
          ).length;
          ...
          const tripTaskPercent =
            tripTasks.length > 0 ? percent(taskCompletedForTrip, tripTasks.length) : null;
          const tripTrainingPercent =
            tripModules.length > 0 ? percent(trainingCompletedForTrip, tripModules.length) : null;
      ...
      const taskPercent = totalTasks > 0 ? percent(completedTasks, totalTasks) : null;
      const trainingPercent = totalTraining > 0 ? percent(completedTraining, totalTraining) : null;
  ...
  const tripMetricsById = Object.fromEntries(
    trips.map((trip) => {
      ...
      const tripTasks = tripTasksByTripId.get(trip.id) || [];
      const tripModules = (modulesByTripId.get(trip.id) || []).filter(isClassroomModuleRow);
      const metricUserIds = getMetricUserIdsForTrip(
        trip.id,
        assignments || [],
        rosterAllRoleUserIdsByTrip
      );
      const completedTasks = metricUserIds.reduce((sum, uid) => {
        return (
          sum +
          tripTasks.filter((task) => isWorkerTaskComplete(completedTaskProgress, trip.id, uid, task))
            .length
        );
      }, 0);
      const completedTraining = metricUserIds.reduce((sum, uid) => {
        return (
          sum +
          tripModules.filter((module) =>
            completedTrainingProgress.get(`${trip.id}:${uid}:${module.id}`)
          ).length
        );
      }, 0);
