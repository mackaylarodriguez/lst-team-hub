...
/** Explicit goal including 0. Empty/invalid → null so callers can fall back. */
export function parseExplicitFundraisingGoal(value) {
  if (value === null || value === undefined || value === "") return null;
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed >= 0 ? parsed : null;
}

/** Personal override (including 0), then roster row, then trip-level default. */
export function resolveWorkerFundraisingGoalAmount({
  ...
  return (
    parseExplicitFundraisingGoal(participant?.fundraisingGoalAmount) ??
    parseExplicitFundraisingGoal(rosterMember?.fundraisingGoalAmount) ??
...
/** Team fundraising total: group mode uses the shared trip amount; otherwise sum worker goals. */
export function computeTeamFundraisingGoalTotal(trip) {
  if (normalizeFundraisingMode(trip?.fundraisingMode) === "team") {
    return parsePositiveFundraisingGoal(trip?.fundraisingGoalAmount) ?? 0;
  }
  return buildTeamFundraisingWorkerRows(trip).reduce(
    (sum, row) => sum + (parseExplicitFundraisingGoal(row.fundraisingGoalAmount) ?? 0),
    0
  );
}
