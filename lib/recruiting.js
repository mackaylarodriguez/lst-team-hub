...
function isMissingRecruitingPhoneColumnError(error) {
  ...
  return error?.code === "42703" && message.includes("recruiting_contacts.phone");
}
...
export async function saveRecruitingContact(values) {
  ...
  if (isMissingRecruitingPhoneColumnError(error)) {
    const fallbackPayload = omitPhoneField(payload);
    const fallbackQuery = values?.id
      ? supabase.from("recruiting_contacts").update(fallbackPayload).eq("id", values.id)
      : supabase.from("recruiting_contacts").insert(fallbackPayload);
