...
function clampYmdPair(startYmd, endYmd) {
  const start = n(startYmd);
  const end = n(endYmd);
...
export function normalizeAvailableRanges(values = {}) {
  const fromList = Array.isArray(values.availableRanges) ? values.availableRanges : [];
  const ranges = [];
  for (const row of fromList) {
    const clamped = clampYmdPair(row?.start || row?.availableStart, row?.end || row?.availableEnd);
    if (clamped) ranges.push(clamped);
  }
  if (!ranges.length) {
    const single = clampYmdPair(values.availableStart, values.availableEnd);
...
async function listLegacyAvailabilityEdits(year) {
  ...
  for (const row of notes || []) {
    ...
    const availableRanges = normalizeAvailableRanges(payload);
    const summary = summarizeAvailableRanges(availableRanges);
...
export async function saveSiteAvailabilityEdit(siteLabel, year, values) {
  ...
  const availableRanges = normalizeAvailableRanges(values);
  const summary = summarizeAvailableRanges(availableRanges);
...
export async function assertTripDatesWithinSiteAvailability({
  ...
  const ranges = normalizeAvailableRanges(row);
