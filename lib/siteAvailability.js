...
export async function saveSiteAvailabilityEdit(siteLabel, year, values) {
  ...
  if (error) {
    ...
    // Older DBs may not have church_name yet — retry without it.
    const msg = String(error?.message || error?.details || "").toLowerCase();
    if (msg.includes("church_name")) {
      const { church_name: _ignored, ...withoutChurch } = payload;
      const retry = await supabase
        .from("site_availability")
        .upsert(withoutChurch, { onConflict: "site_name,year" })
        .select("*")
        .single();
      if (retry.error) {
        console.error("Error saving site availability", retry.error);
        throw retry.error;
      }
      const normalized = normalizeAvailabilityRow(retry.data);
      return { ...normalized, churchName };
    }
