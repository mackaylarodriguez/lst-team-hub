...
export async function loadTripEmailContext(admin, tripId) {
  const { data: trip, error: tripError } = await admin
    .from("trips")
    .select(
      "id, trip_name, location, host, start_date, end_date, project_length_summary, fundraising_goal_amount, extra_travel_status, trip_fee_amount, materials_fee_amount, hannover_housing_fee_amount"
    )
    .eq("id", tripId)
    .maybeSingle();
  ...
  return {
    tripId,
    tripName: normalizeText(trip?.trip_name) || "Untitled trip",
