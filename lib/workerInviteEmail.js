...
import { buildEmailCtaBlock, buildEmailHtmlHead, EMAIL_BRAND_ACCENT } from "@/lib/emailCtaButton";
...
function emailSectionCard(title, innerHtml) {
  const titleHtml = title
    ? `<div style="font-weight:700;margin:0 0 12px;font-size:15px;color:${EMAIL_BRAND_ACCENT}">${escapeHtml(title)}</div>`
    : "";
  return `
...
function wrapWorkerInviteEmailCard({ title, bodyHtml, ctaHtml, footerNote }) {
...
export function buildWorkerInviteEmailHtml({
  ...
  const hubTripUrl = String(tripLoginUrl || loginUrl).trim();

  const projectInfoCard = emailSectionCard(
    "Your project",
    `
      <p style="margin:0 0 12px;font-size:15px;font-weight:700;color:${EMAIL_BRAND_ACCENT}">${escapeHtml(lockLine)}</p>
  ...
  const gettingStartedCta = inviteUrl
    ...
    : hubTripUrl
      ? `<div style="margin:14px 0 4px">${buildEmailCtaBlock({ href: hubTripUrl, label: "Log in to the Hub" })}</div>`
      : "";

  const accountSetupStep = inviteUrl
    ? `<li style="margin-bottom:6px">Set up your password and complete your profile</li>`
    : `<li style="margin-bottom:6px">Log in with your email and password to view your trip</li>`;
  ...
  const footerNote = inviteUrl
    ? undefined
    : "Let's Start Talking (LST) · Sign in with the email already on file. Use Forgot Password on the login page if you need help.";
