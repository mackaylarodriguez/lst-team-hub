...
const BASIC_TRAINING_REGISTER_URL = String(
  process.env.WORKER_INVITE_BASIC_TRAINING_URL ||
    "https://lst.app.neoncrm.com/np/clients/lst/survey.jsp?surveyId=134&"
).trim();
const GATEWAY_TRAINING_REGISTER_URL = String(
  process.env.WORKER_INVITE_GATEWAY_TRAINING_URL ||
    "https://lst.app.neoncrm.com/np/clients/lst/survey.jsp?surveyId=136&"
).trim();
const TRAINING_DATES_FORM_URL = String(
  process.env.WORKER_INVITE_TRAINING_DATES_URL ||
    "https://lst.app.neoncrm.com/np/clients/lst/survey.jsp?surveyId=136&"
).trim();
const ON_DEMAND_TRAINING_URL = String(
  process.env.WORKER_INVITE_ON_DEMAND_TRAINING_URL ||
...
export function buildWorkerInviteEmailHtml({
  ...
  const actionItemsCard = emailSectionCard(
    ...
    `
      ...
      <p style="margin:0 0 10px;font-size:14px">
        <strong>On-demand training</strong> — Video-based training you can complete on your own schedule.
        ${registerLink("Open on-demand training.", ON_DEMAND_TRAINING_URL)}
      ...
      <ul style="margin:0 0 0 18px;padding:0;font-size:14px;line-height:1.55">
        <li style="margin-bottom:10px">
          ...
          ${registerLink("Click here to register.", BASIC_TRAINING_REGISTER_URL)}
        </li>
        <li style="margin-bottom:10px">
          <strong>Gateway training:</strong> Mandatory for the whole team; we prefer you attend the same session.
          ${trainingDatesNote} Complete in the month or two before you leave.
          ${registerLink("Click here to register.", GATEWAY_TRAINING_REGISTER_URL)}
        </li>
        <li style="margin-bottom:0">
          <strong>Endmeeting:</strong> Final session after your project ends. Use the same Gateway Training link to register.
        </li>
      </ul>
    `.trim()
  );
