...
/**
 * Merge duplicate site_budget_notes rows (same canonical site) and remove rows with no
 * housing note, workbook notes, or logistics URL. Returns the refreshed list and how many rows were deleted.
 */
export async function cleanupSiteBudgetNotesRows() {
  ...
  for (const note of rows) {
    ...
    if (emptyHousing && emptyWb && emptyLog) {
...
export async function getBudgetAverages() {
  ...
  // Airfare card: mean of trip_tickets.total_ticket_cost only. Ignores amount_worker_paid,
  // total_lst_cost, hp_total_charge, and every other column on the row.
  ...
  // Housing cards: only trip_budgets.housing_amount. Ignores budget_amount, returned_amount, etc.
  ...
  if (tripIdsForH2.size === 0) {
    return {
      ...
      housing2: { average: null, count: 0, budgetPerTeam: HOUSING1_BUDGET_PER_TEAM },
    };
  }
  ...
  // Still housing_amount only; missing values count as 0 in this average (non-YF trips only).
  ...
  return {
    ...
    housing1: { average: housing1Average, count: housing1Values.length, budgetPerTeam: HOUSING1_BUDGET_PER_TEAM },
    housing2: {
      average: housing2Average,
      count: housing2Values.length,
      budgetPerTeam: HOUSING1_BUDGET_PER_TEAM,
    },
  };
}
