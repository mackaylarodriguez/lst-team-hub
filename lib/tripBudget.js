...
export async function saveTripBudget(tripId, values) {
  ...
  const merged = { ...(prior || {}), ...values, tripId };
...
export async function listAllTripBudgets() {
  ...
  return (data || []).map((row) => ({
    ...
    notes: n(row.notes),
    numWorkers: row.num_workers ?? null,
...
/**
 * Group housing budget rows by site label (budget site fields, else trip location).
 * Used on the Sites / materials page.
 */
export function groupBudgetRowsBySiteForMaterials(budgetRows, trips) {
  const tripById = new Map((trips || []).map((t) => [t.id, t]));
  const groups = new Map();

  for (const row of budgetRows || []) {
    const trip = tripById.get(row.tripId);
    const siteParts = [n(row.siteCountry), n(row.siteCity)].filter(Boolean);
    const label =
