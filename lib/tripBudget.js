...
/**
 * Housing budget amount is staff-entered only (housing_budget_amount).
 * Never fall back to budget_amount (historically filled with fundraising totals).
 * If both columns hold the same number, treat as unset — leftover from that old sync.
 */
function resolveHousingBudgetAmount(housingBudgetRaw, budgetAmountRaw) {
  const housing = n(housingBudgetRaw);
  if (!housing) return "";
  const budget = n(budgetAmountRaw);
  if (!budget) return housing;
  const housingNum = parseAmount(housing);
  const budgetNum = parseAmount(budget);
  if (housingNum != null && budgetNum != null && housingNum === budgetNum) return "";
  return housing;
}
...
function isMissingWorkbookNotesUpdatedAtColumnError(error) {
  const msg = String(error?.message || error?.details || "").toLowerCase();
  return (
    msg.includes("workbook_notes_updated_at") &&
    (msg.includes("does not exist") || msg.includes("schema cache") || error?.code === "42703")
  );
}
...
export async function getTripBudget(tripId) {
  ...
  return {
    ...
    housingBudgetAmount: resolveHousingBudgetAmount(data.housing_budget_amount, data.budget_amount),
...
export async function listAllTripBudgets() {
  ...
  return (data || []).map((row) => ({
    ...
    housingBudgetAmount: resolveHousingBudgetAmount(row.housing_budget_amount, row.budget_amount),
...
export async function listSiteBudgetNotes(options = {}) {
  const includeAvailabilityStorage = Boolean(options?.includeAvailabilityStorage);
  const { data, error } = await supabase
  ...
  if (includeAvailabilityStorage) return rows;
  return rows.filter((row) => !isSiteAvailabilityStorageName(row.siteName));
}
...
export async function saveSiteHousingNoteForSiteLabel(siteLabel, notesText) {
  ...
  if (isSiteAvailabilityStorageName(label)) {
    throw new Error(
      "Availability data cannot be saved as a budget note. Use Sites → Availability instead."
    );
  }
...
export async function updateSiteBudgetNote(id, values) {
  if (isSiteAvailabilityStorageName(values?.siteName)) {
    throw new Error(
      "Availability data cannot be saved as a budget note. Use Sites → Availability instead."
    );
  }
  ...
  if (
    error &&
    isMissingWorkbookNotesUpdatedAtColumnError(error) &&
    Object.prototype.hasOwnProperty.call(payload, "workbook_notes_updated_at")
  ) {
    const { workbook_notes_updated_at, ...rest } = payload;
    payload = rest;
    ({ data, error } = await runUpdate(payload));
  }
 ...
 * Fee = trip Fee + Materials Fee (+ domestic fees when applicable);
 * leftover = fundraising − team budget − airfare − housing − fee + returned.
 */
export function computeStaffTeamBudgetExpenseSummary({
  ...
  const returnedTotal = parseBudgetAmountOrNull(budgetRow?.returnedAmount) ?? 0;
  const feeBreakdown = getTripBudgetFeeBreakdown(trip, { isDomestic });
  const feeTotal = feeBreakdown.total;
  const spentTotal = (teamBudget ?? 0) + base.airfareTotal + base.housingTotal + feeTotal;
  const leftover = fundraising == null ? null : fundraising - spentTotal + returnedTotal;
...
export async function upsertSiteBudgetNote(values) {
  const siteName = n(values.siteName);
  if (isSiteAvailabilityStorageName(siteName)) {
    throw new Error(
      "Availability data cannot be saved as a budget note. Use Sites → Availability instead."
    );
  }
  ...
  if (
    error &&
    isMissingWorkbookNotesUpdatedAtColumnError(error) &&
    Object.prototype.hasOwnProperty.call(row, "workbook_notes_updated_at")
  ) {
    const { workbook_notes_updated_at, ...rest } = row;
    ({ data, error } = await runUpsert(rest));
  }
