...
export async function getBudgetAverages() {
  const { data, error } = await supabase.from("trip_budgets").select("housing_amount");

  if (error) throw error;

  const housingValues = (data || [])
 ...
 * Always Fee + Materials Fee; if domestic (Massachusetts), also Domestic Project + Domestic Fee + Domestic Materials Fee.
 */
export function sumTripBudgetFeeAmount(trip, { isDomestic = null } = {}) {
  ...
  const parts = [
    parseBudgetAmountOrNull(trip?.tripFeeAmount),
    parseBudgetAmountOrNull(trip?.materialsFeeAmount),
  ];
  if (domestic) {
    parts.push(
      parseBudgetAmountOrNull(trip?.domesticProjectFeeAmount),
      parseBudgetAmountOrNull(trip?.domesticFeeAmount),
      parseBudgetAmountOrNull(trip?.domesticMaterialsFeeAmount)
    );
  }
 ...
 * Fee = trip fees (+ domestic fees when applicable); leftover = fundraising − airfare − housing − fee.
 */
export function computeStaffTeamBudgetExpenseSummary({
  ...
  const feeTotal = sumTripBudgetFeeAmount(trip, { isDomestic });
