...
export async function listTripTasks(tripId) {
  ...
  if (existingTasks.length > 0 && hasDefaultWorkerTasks) {
    ...
    const insertedMissing = await insertMissingWorkerTemplateTasks(
      tripId,
      existingTasks,
      trip
    );
    const merged = [...existingTasks, ...insertedMissing];
    return merged.map(normalizeTripTask).sort(compareTripTasks);
  }
...
function seededWorkerTaskTitle(task, location) {
  ...
  if (task.id === "worker-task-fundraising-2000") {
    return "50% of fundraising due 90 days before departure";
  }
  if (task.id === "worker-task-fundraising-all") {
    return "50% of fundraising due 30 days prior to departure";
  }
  return task.title;
}

function seededWorkerTaskDescription(task) {
  const fromDetails = String(task?.details || "").trim();
  if (fromDetails) return fromDetails;
  const fromDesc = String(task?.description || "").trim();
  return fromDesc || null;
}

function rowMatchesWorkerTemplate(row, template, location) {
  if (String(row?.category || "").trim() !== "worker_default") return false;
  const expectedTitle = seededWorkerTaskTitle(template, location);
  if (normalizeTaskTitleKey(row.title) === normalizeTaskTitleKey(expectedTitle)) return true;
  const hit = findWorkerTaskTemplate({ title: row.title });
  return hit?.id === template.id;
}

async function insertMissingWorkerTemplateTasks(tripId, existingRows, trip) {
  const location = trip?.location || "";
  const startDate = trip?.start_date || null;
  const trainingTimelineType = trip?.training_timeline_type || null;
  const missing = WORKER_TASK_TEMPLATE.filter(
    (template) => !existingRows.some((row) => rowMatchesWorkerTemplate(row, template, location))
  );
  ...
  const payload = missing.map((task) => ({
    ...
    description: seededWorkerTaskDescription(task),
  ...
  const { data, error } = await supabase.from("trip_tasks").insert(payload).select("*");
  if (error) {
    console.error("Error inserting missing worker template tasks", error);
    throw error;
  }
  return data || [];
}
