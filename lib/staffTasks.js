...
export function loadStaffTasks(trip) {
  if (!trip) return [];
  if (typeof window === "undefined") return sortStaffTasksByTemplate(trip.staffTasks || []);

  try {
    const saved = localStorage.getItem(staffTasksKey(trip.id));
    const tasks = saved ? JSON.parse(saved) : (trip.staffTasks || []);
    return sortStaffTasksByTemplate(tasks);
  } catch {
...
export function saveStaffTasks(tripId, tasks) {
  ...
  localStorage.setItem(staffTasksKey(tripId), JSON.stringify(orderedTasks));
