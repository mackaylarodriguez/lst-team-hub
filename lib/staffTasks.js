...
export function computeStaffTaskDueDate(task, trip) {
  ...
  const title = String(task.taskName || task.title || "").toLowerCase();
  const workArea = String(task.workArea || "").toLowerCase();
  const sequence = Number(task.sequence || 0);

  // Exceptions first
  if (title.includes("social media")) {
    // Later: treat as category 3 (about 3 months before departure)
    return startDate ? addDays(startDate, -90) : "";
  }

  if (
    title.includes("training launch") ||
    title.includes("training") ||
    title.includes("visa") ||
    title.includes("entry") ||
    workArea === "travel" ||
    title.includes("housing addressed")
  ) {
    // Earlier: treat as category 2 (about 3 weeks after locking/creating)
    return createdAt ? addDays(createdAt, 21) : "";
  }

  // Base rules by sequence bucket
  switch (sequence) {
    case 1:
      // Needs to happen upon locking a team (about a week after creating trip)
      return createdAt ? addDays(createdAt, 7) : "";
    case 2:
      // Within weeks of locking (about 3 weeks after creating trip)
      return createdAt ? addDays(createdAt, 21) : "";
    case 3:
      // Within months of departure (about 3 months before start)
      return startDate ? addDays(startDate, -90) : "";
    case 4:
      // During the project (use project start date)
      return startDate || "";
    case 5:
      // Post project (use project end date)
      return endDate || "";
    default:
      return "";
  }
