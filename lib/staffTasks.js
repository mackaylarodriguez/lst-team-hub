...
export async function listStaffTasksForTrip(tripId) {
  ...
  if (existingTasks.length > 0) {
    const insertedMissing = await insertMissingStaffTemplateTasks(tripId, existingTasks);
    const merged = insertedMissing.length
      ? [...existingTasks, ...insertedMissing]
      : existingTasks;
    console.log("[staffTasks] listStaffTasksForTrip loaded from database", {
      tripId,
      rowCount: merged.length,
      insertedMissing: insertedMissing.length,
    });
    return sortStaffTasksByTemplate(normalizeLoadedStaffTasks(tripId, merged.map(normalizeStaffTaskRow)));
  }
...
/**
 * Template staff tasks follow rule-computed due dates (so rule changes apply).
 * Custom / non-template tasks keep any stored due date.
 */
export function effectiveStaffTaskDueDate(task, trip) {
  const computed = toCalendarDatePart(computeStaffTaskDueDate(task, trip));
  const stored = toCalendarDatePart(task?.dueDate);
  if (findStaffTaskTemplate(task)) {
    return computed || stored || "";
  }
  return stored || computed || "";
}

export function applyStaffTaskDueDateRules(tasks, trip) {
...
/** Persist rule-driven due dates when they differ from what's stored (template tasks only). */
export async function syncStaffTaskDueDatesForTrip(tripId, trip, tasks) {
  const nextTasks = applyStaffTaskDueDateRules(tasks, trip);
  const updates = [];

  for (let i = 0; i < (tasks || []).length; i += 1) {
    const task = tasks[i];
    if (!findStaffTaskTemplate(task)) continue;
    const before = toCalendarDatePart(task.dueDate);
    const after = toCalendarDatePart(nextTasks[i]?.dueDate);
    if (before === after) continue;
    if (!task.id) continue;
    updates.push({ id: task.id, due_date: after || null });
  }

  if (updates.length === 0) return nextTasks;
  ...
  for (const update of updates) {
    const { error } = await supabase
      ...
      .eq("trip_id", tripId);
    if (error) {
      console.error("[staffTasks] syncStaffTaskDueDatesForTrip update failed", {
        tripId,
        update,
        error,
      });
...
export function computeStaffTaskDueDate(task, trip) {
  ...
  if (template?.id === "materials-sent") {
    // 42 days before project start
    return startDate ? subtractDays(startDate, 42) : "";
  }
  if (template?.id === "budget-created") {
    // 49 days before project start
    return startDate ? subtractDays(startDate, 49) : "";
  }
  if (template?.id === "money-sent") {
    // 28 days before project start
    return startDate ? subtractDays(startDate, 28) : "";
  }
  if (template?.id === "team-training") {
    // 28 days before project start
    return startDate ? subtractDays(startDate, 28) : "";
  }
  if (
    template?.id === "record-keeping" &&
    String(template.workArea || "").trim().toLowerCase() === "materials"
  ) {
    // 42 days before project start
    return startDate ? subtractDays(startDate, 42) : "";
  }
  if (template?.id === "record-keeping-sent") {
    // 28 days before project start
    return startDate ? subtractDays(startDate, 28) : "";
  }
  if (template?.id === "export-hub-project-data-365") {
    // ~30 days after project end / return
    return endDate ? addDays(endDate, 30) : "";
  }
...
async function insertMissingStaffTemplateTasks(tripId, existingRows) {
  const missing = STAFF_TASK_TEMPLATE.filter((template) => {
    const expectedId = buildStaffTaskId(tripId, template);
    const hasId = (existingRows || []).some((row) => String(row?.id || "") === expectedId);
    if (hasId) return false;
    return !(existingRows || []).some((row) => {
      const areaMatch =
        String(row?.work_area || row?.workArea || "")
          .trim()
          .toLowerCase() === String(template.workArea || "").trim().toLowerCase();
      const rowName = String(row?.task_name || row?.taskName || "")
        .trim()
        .toLowerCase();
      const nameMatch =
        rowName === String(template.taskName || "").trim().toLowerCase() ||
        (template.legacyTaskNames || []).some(
          (legacy) => String(legacy || "").trim().toLowerCase() === rowName
        );
  ...
  const payload = missing.map((task) => ({
    id: buildStaffTaskId(tripId, task),
    trip_id: tripId,
    work_area: task.workArea,
    sequence: task.sequence,
  ...
  const { data, error } = await supabase.from("trip_staff_tasks").insert(payload).select("*");
  if (error) {
    console.error("Error inserting missing staff template tasks", error);
    return [];
  }
  return data || [];
}
...
function normalizeLoadedStaffTasks(tripId, tasks) {
  return dedupeStaffTasks(
    (tasks || []).map((task, index) => {
      ...
      const templateNotes = String(templateMatch?.notes || "").trim();
      const rowNotes = String(task?.notes || "").trim();
      // Always surface template default notes when a row has none (covers older seeded trips).
      const notes = rowNotes || templateNotes || "";

      const currentName = String(task?.taskName || task?.title || "").trim();
      const legacyNames = (templateMatch?.legacyTaskNames || []).map((n) =>
        String(n || "").trim().toLowerCase()
      );
      // Rename rows still stored under a legacy template title.
      const taskName =
        templateMatch &&
        legacyNames.includes(currentName.toLowerCase())
          ? templateMatch.taskName
          : task.taskName;
