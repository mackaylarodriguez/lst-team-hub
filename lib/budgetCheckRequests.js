...
/** Default Donna / staff-task due date: two weeks from today (local calendar). */
export function defaultBudgetCheckDueDateIso() {
  const d = new Date();
...
export async function submitBudgetCheckRequest({ tripId, amount, note, dueDate }) {
...
export async function updateBudgetCheckRequest({ id, amount, note, payee }) {
  const token = await getAccessTokenForApi();
  const res = await fetch("/api/budget-check-request", {
    method: "PATCH",
    headers: {
