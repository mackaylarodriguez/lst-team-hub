...
import { toPgDateOrNull, coerceSqlDateToYmd } from "./isoDateYmd";
import { parsePositiveFundraisingGoal, parseExplicitFundraisingGoal } from "./tripFundraising";
...
import { listRegisteredWorkersForPicker, resolveRegisteredWorker } from "./workerLookup";
...
async function syncProfileContactToTripRosters({ profileId, phone, tshirtSize }) {
  ...
  const res = await fetch("/api/sync-profile-roster-contact", {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      tshirtSize: String(tshirtSize ?? "").trim(),
    }),
  });
  ...
  if (!res.ok) {
    throw new Error(
      json.error || "Profile saved, but trip roster phone could not be updated."
    );
  }
...
function buildTripTeamMemberRows(teamMembers) {
  return (teamMembers || [])
    .map((member) => {
      ...
      const startDate = toPgDateOrNull(member?.startDate) || "";
      const endDate = toPgDateOrNull(member?.endDate) || "";
...
export async function createTripForCurrentUser({
  ...
  const lockLocation = String(location || "").trim();
  if (lockLocation) {
    const { assertTripDatesWithinSiteAvailability } = await import("@/lib/siteAvailability");
    const availabilityCheck = await assertTripDatesWithinSiteAvailability({
      siteLabel: lockLocation,
      startDate,
      endDate,
    });
...
export async function ensureWorkerProfilesForTripRoster(tripId) {
  ...
  try {
    registeredWorkers = await listRegisteredWorkersForPicker();
  } catch (lookupError) {
    console.error("Unable to load registered workers for roster profile seeding", lookupError);
  }

  for (const m of members || []) {
    let em = normalizeEmail(m.email);
    let matchedProfile = null;

    if (!em) {
      matchedProfile = resolveRegisteredWorker(registeredWorkers, {
        firstName: m.first_name,
        lastName: m.last_name,
      });
      if (matchedProfile?.email) {
        em = normalizeEmail(matchedProfile.email);
      }
    }
...
/** Staff/admin only: group leader contact for travel (saved from the trip Travel Form tab). */
...
export async function updateProfilePhoneAndTshirtSize({
  ...
  if (includePhoneAndTshirt) {
    await syncProfileContactToTripRosters({
      profileId: id,
      phone: patch.phone,
      tshirtSize: patch.tshirt_size,
    });
  }
...
export async function listWorkerAssignmentSummary() {
  ...
  const workers = (profiles || []).filter((worker) => {
    const role = normalizeRole(worker.role);
    // Treat blank role as worker (legacy / claim-in-progress rows).
  ...
  /** Union `trip_assignments` rows for every profile id in the cohort (same normalized email). */
  function mergeTripAssignmentsForProfileIds(profileIds) {
    const byTripId = new Map();
    for (const uid of profileIds) {
      for (const a of assignmentsByUserId.get(uid) || []) {
        if (!a?.tripId) continue;
        if (!byTripId.has(a.tripId)) byTripId.set(a.tripId, a);
      }
    }
    return [...byTripId.values()];
  }

  /** One row per email: duplicate worker profiles with the same email used to overwrite each other and drop trips. */
  const workerProfilesByEmail = new Map();
  (workers || []).forEach((worker) => {
    const email = normalizeEmail(worker.email);
    if (!email) return;
    if (!workerProfilesByEmail.has(email)) workerProfilesByEmail.set(email, []);
    workerProfilesByEmail.get(email).push(worker);
  });

  workerProfilesByEmail.forEach((cohort, email) => {
    cohort.sort((a, b) => String(a.id).localeCompare(String(b.id)));
    const primary = cohort[0];
    summaryByEmail.set(email, {
      ...
      hasAccount: true,
      invitePending: false,
      assignments: mergeTripAssignmentsForProfileIds(cohort.map((w) => w.id)),
    });
  });
  ...
  function dedupeAssignmentsByTripId(list) {
    const byTripId = new Map();
    for (const a of list || []) {
      if (!a?.tripId) continue;
      if (!byTripId.has(a.tripId)) byTripId.set(a.tripId, a);
    }
    return [...byTripId.values()].sort((left, right) =>
  ...
  return [...summaryByEmail.values()]
    .map((worker) => ({
      ...worker,
      assignments: dedupeAssignmentsByTripId(worker.assignments),
    }))
    .sort((left, right) => left.name.localeCompare(right.name));
}

/** Worker profiles keyed by normalized email — used to show roster account status (matches Staff worker list). */
export async function loadWorkerProfilesByEmails(emails) {
  ...
  await Promise.all(
    unique.map(async (email) => {
      ...
      if (error) {
        console.error("Error loading worker profile for roster email", email, error);
        return;
      }
...
/** Trip id → assignment count (paginated). Used with roster headcount like trip card Workers. */
export async function listTripAssignmentCountsByTripId() {
  const pageSize = 1000;
  const counts = {};
  let from = 0;
  for (;;) {
    ...
    if (error) {
      console.error("Error loading trip assignment counts", error);
      throw error;
    }
    const batch = data || [];
...
export async function listTripParticipants(tripId) {
  ...
  let tripFundraisingGoalAmount = null;
  try {
    const { data: tripRow, error: tripError } = await supabase
      .from("trips")
      .select("fundraising_goal_amount")
      .eq("id", tripId)
      .maybeSingle();
    if (!tripError && tripRow) {
      tripFundraisingGoalAmount = tripRow.fundraising_goal_amount;
    }
  } catch (error) {
    console.error("Unable to load trip fundraising goal for participants", error);
  }
  const tripDefaultFundraisingGoal = parsePositiveFundraisingGoal(tripFundraisingGoalAmount);
  ...
  return userIds.map((userId) => {
    ...
    const goalFromRoster = parseExplicitFundraisingGoal(tm?.fundraisingGoalAmount);
    const effectiveFundraisingGoal = goalFromRoster ?? tripDefaultFundraisingGoal;
    ...
    return {
      ...
      fundraisingGoalAmount: effectiveFundraisingGoal ?? undefined,
    };
  });
}
...
function normalizeTrip(trip) {
  ...
  const startDate = coerceSqlDateToYmd(row.start_date);
  const endDate = coerceSqlDateToYmd(row.end_date);
  const tripName =
    String(
      row.trip_name ||
      row.tripName ||
