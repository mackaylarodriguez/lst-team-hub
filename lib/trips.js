...
/**
 * Update phone and T-shirt size on a profile. Allowed for the signed-in user on their own row,
 * or for admin/staff on any profile. Matching trip roster rows (same email) get cell_phone / tshirt_size synced.
 */
export async function updateProfilePhoneAndTshirtSize({ profileId, phone, tshirtSize }) {
  ...
  const normalizedTshirt = normalizeTshirtSizeForSelect(tshirtSize || "") || null;

  const { data: updated, error: updateError } = await supabase
    .from("profiles")
    .update({
      phone: normalizedPhone,
      tshirt_size: normalizedTshirt,
    })
    .eq("id", id)
    .select("id, phone, tshirt_size")
    .single();

  if (updateError) {
    console.error("Error updating profile phone / t-shirt size", updateError);
    throw updateError;
  }
