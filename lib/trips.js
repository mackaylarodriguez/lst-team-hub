  ...
  if (profile?.id) {
    ...
    if (existingProfile?.id) {
      ...
      const { data: profileByEmail } = await supabase
        ...
        .limit(1)
        .maybeSingle();
      if (profileByEmail?.id) profileIdToUse = profileByEmail.id;
    }
    ...
    if (profileIdToUse) {
      ...
      console.warn(
        "Skipping trip_assignments insert: no profiles row for current user - trip was created but current user was not assigned. Add yourself from the trip Team tab if needed."
      );
    }
  }
...
export async function listTripParticipants(tripId) {
  ...
  const fundraisingGoalByEmail = new Map(
    (teamMembers || [])
      .filter((m) => m?.email && m?.fundraisingGoalAmount != null && Number(m.fundraisingGoalAmount) > 0)
      .map((m) => [normalizeEmail(m.email), Number(m.fundraisingGoalAmount)])
  );
  ...
  return userIds.map((userId) => {
    ...
    const fundraisingGoalAmount = fundraisingGoalByEmail.get(normalizeEmail(email)) ?? null;
    ...
    return {
      ...
      fundraisingGoalAmount: fundraisingGoalAmount != null ? fundraisingGoalAmount : undefined,
    };
  });
}
...
export async function deleteTrip(tripId) {
  ...
  if (!deletedTrips || deletedTrips.length === 0) {
    throw new Error(
      "Trip delete was blocked by the live database. Run the latest trip delete policy SQL in Supabase and try again."
    );
  }
