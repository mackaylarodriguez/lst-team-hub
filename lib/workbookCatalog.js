 ...
 * Order: Blue Luke (legacy) → Beginner (Beg 1–2) → Core → Discover → Advanced.
 * Keys are stable for parsing saved inventory strings.
 */
import { resolveWorkbookInventoryKey } from "@/lib/workbookInventory";
...
export const WORKBOOK_REFERENCE_COLUMNS = [
  { key: "blue luke", label: "BLUE LUKE", series: "blueLuke" },
  { key: "beg 1", label: "BEG 1", series: "beginner" },
  { key: "beg 2", label: "BEG 2", series: "beginner" },
...
export function workbookNameToCanonicalKey(name) {
  return resolveWorkbookInventoryKey(name);
}
