...
export async function logTripActivity({
  ...
  if (error) {
    ...
    // RLS: often profile.email ≠ roster email while JWT email matches; deploy updated private_trip_access_helpers.sql
    if (
      code === "42501" ||
      msg.includes("row-level security") ||
      (msg.includes("policy") && msg.includes("trip_activity"))
    ) {
      console.warn(
        "[trip_activity] Insert blocked by RLS. Run updated supabase/private_trip_access_helpers.sql in Supabase (JWT email vs roster match)."
      );
      return null;
    }
    throw error;
