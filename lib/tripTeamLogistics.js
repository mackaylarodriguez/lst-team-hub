...
/** Team Materials tab: accountant, recorder, and ship-to address filled in. */
export function isTeamMaterialsLogisticsComplete(raw) {
  const row = normalizePayload(raw);
  return !!(row.teamAccountant && row.teamRecorder && row.materialsShipAddress);
}
