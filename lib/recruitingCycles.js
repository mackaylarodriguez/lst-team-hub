...
import { enrichTeamMembersWithExistingWorkers } from "@/lib/workerLookup";
...
export async function listRecruitingYears() {
  ...
  const years = new Set([2027, 2028, currentYear, currentYear + 1]);
  (data || []).forEach((row) => {
...
export async function updateRecruitingContactActivityLog({
  activityId,
  actionType,
  actionDate,
  summary,
  ...
  if (!id) {
    throw new Error("Contact activity is required.");
  }
  ...
  if (staffMember) {
    patch.staff_member = normalizeNullableText(staffMember);
  }
  ...
  if (error) {
    console.error("Error updating recruiting contact activity log", error);
    throw error;
  }

  emitRecruitingUpdated({
    recruitingCycleContactId: data?.recruiting_cycle_contact_id,
...
export async function promoteRecruitingRecordToPotentialTeam(record, { staffMember } = {}) {
  ...
  await logRecruitingActivity({
    recruitingCycleContactId: data.id,
    actionType: "promote",
    actionDate: new Date().toISOString(),
    staffMember,
    summary: "Moved to Potential Teams",
  });

  emitRecruitingUpdated({ recruitingCycleContactId: data.id, action: "promoted" });
...
export async function moveRecruitingRecordToYear(record, targetYear, { staffMember } = {}) {
  if (!record?.id) {
    throw new Error("Recruiting record not found.");
  }
  ...
  if (!Number.isFinite(year) || year < 2000) {
    throw new Error("Choose a valid recruiting year.");
  }

  const currentYear = Number(record.recruitingYear || 0);
  if (currentYear === year) {
    return record;
  }
  ...
  if (!contactId) {
    throw new Error("Recruiting contact is missing.");
  }
  ...
  const { data: existing, error: existingError } = await supabase
    ...
    .eq("recruiting_year", year)
    .neq("id", record.id)
    .maybeSingle();

  if (existingError) {
    console.error("Error checking recruiting year conflict", existingError);
    throw existingError;
  }

  if (existing?.id) {
    throw new Error(`This contact already has a recruiting record for ${year}.`);
  }
  ...
  if (error) {
    console.error("Error moving recruiting record to year", error);
    throw error;
  }

  await logRecruitingActivity({
    recruitingCycleContactId: data.id,
    actionType: "year_move",
    actionDate: new Date().toISOString(),
    staffMember,
    summary: `Moved from ${currentYear || "—"} to ${year}`,
  });

  emitRecruitingUpdated({ recruitingCycleContactId: data.id, action: "year_moved" });
...
export async function demoteRecruitingRecordToOutreach(record, { staffMember } = {}) {
  if (!record?.id) {
    throw new Error("Recruiting record not found.");
  }

  const outreachSibling = await findRecruitingCycleSibling(record, { isPotentialTeam: false });
  if (outreachSibling?.id) {
    await deleteRecruitingCycleContact(record.id);
    await logRecruitingActivity({
      recruitingCycleContactId: outreachSibling.id,
      actionType: "demote",
      actionDate: new Date().toISOString(),
      staffMember,
      summary: "Removed Potential Teams copy",
    });
    emitRecruitingUpdated({ recruitingCycleContactId: outreachSibling.id, action: "demoted" });
    return { id: outreachSibling.id };
  }

  const { data, error } = await supabase
    .from("recruiting_cycle_contacts")
    .update({
      is_potential_team: false,
      updated_at: new Date().toISOString(),
    })
    .eq("id", record.id)
    .select("*")
  ...
  if (error) {
    console.error("Error moving recruiting record back to outreach list", error);
    throw error;
  }
  ...
  await logRecruitingActivity({
    ...
    actionDate: new Date().toISOString(),
    staffMember,
    summary: "Moved to Recruiting list",
  });

  emitRecruitingUpdated({ recruitingCycleContactId: record.id, action: "demoted" });
...
export async function convertRecruitingCycleRecordToTrip({
  ...
  const enrichedTeamMembers = Array.isArray(teamMembers)
    ? await enrichTeamMembersWithExistingWorkers(teamMembers)
    : teamMembers;
  const normalizedTeamMembers = normalizeTripTeamMembers(enrichedTeamMembers, record.contact);
  ...
  const trip = await createTripForCurrentUser({
    ...
    hannoverHousingFeeAmount: hannoverHousingFeeAmount || "",
...
async function findRecruitingCycleSibling(record, { isPotentialTeam }) {
  const contactId = record?.contactId || record?.contact_id || "";
  const recruitingYear = Number(record?.recruitingYear || record?.recruiting_year || 0);
  const recordId = record?.id || "";
  if (!contactId || !recruitingYear || !recordId) return null;
  ...
  const { data, error } = await supabase
    ...
    .eq("contact_id", contactId)
    .eq("recruiting_year", recruitingYear)
    .eq("is_potential_team", isPotentialTeam === true)
    .neq("id", recordId)
    .maybeSingle();

  if (error) {
    console.error("Error loading recruiting cycle sibling", error);
    throw error;
  }
...
/**
 * Delete a recruiting row from a specific board without removing the other list's copy.
 * - Potential Teams delete: removes only the potential row when a recruiting-list copy exists;
 *   otherwise moves the row back to the Recruiting list instead of deleting it entirely.
 * - Recruiting list delete: removes only the outreach row when a potential-team copy exists.
 */
export async function deleteRecruitingCycleContactForBoard(id, board = "outreach", { staffMember } = {}) {
  const { data: record, error: loadError } = await supabase
    .from("recruiting_cycle_contacts")
    .select("id, contact_id, recruiting_year, is_potential_team, is_converted_to_team")
    .eq("id", id)
    .maybeSingle();

  if (loadError) {
    console.error("Error loading recruiting cycle contact before board delete", loadError);
    throw loadError;
  }
  ...
  if (record.is_converted_to_team) {
    throw new Error("Unlock the locked team before deleting this row.");
  }

  const normalizedBoard = String(board || "outreach").trim().toLowerCase();
  const normalizedRecord = {
    id: record.id,
    contactId: record.contact_id,
    recruitingYear: Number(record.recruiting_year || 0),
    isPotentialTeam: !!record.is_potential_team,
  };

  if (normalizedBoard === "potential" && normalizedRecord.isPotentialTeam) {
    const outreachSibling = await findRecruitingCycleSibling(normalizedRecord, {
    ...
    await demoteRecruitingRecordToOutreach(normalizedRecord, { staffMember });
    return { action: "demoted" };
  }

  if (normalizedBoard === "outreach" && !normalizedRecord.isPotentialTeam) {
    const potentialSibling = await findRecruitingCycleSibling(normalizedRecord, {
      isPotentialTeam: true,
    });
    if (potentialSibling?.id) {
      await deleteRecruitingCycleContact(id);
      return { action: "deleted" };
    }
  }
...
export async function importRecruitingContacts({ recruitingYear, rows, destination, staffMember }) {
  ...
  const genderUpdates = validRows.filter((row) => row.email && row.gender);
  if (genderUpdates.length > 0) {
    await Promise.all(
      genderUpdates.map(async (row) => {
        const contact = contactByEmail.get(row.email);
        if (!contact?.id) return;
        const { error: genderUpdateError } = await supabase
          .from("recruiting_contacts")
          .update({ gender: normalizeNullableText(row.gender) })
          .eq("id", contact.id);
        if (genderUpdateError) {
          console.error("Error updating recruiting contact gender during import", genderUpdateError);
        }
      })
    );
  }
