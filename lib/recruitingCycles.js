...
export async function promoteRecruitingRecordToPotentialTeam(record, { staffMember } = {}) {
  ...
  const { data: existingPotential, error: existingError } = await supabase
    .from("recruiting_cycle_contacts")
    .select("id")
    .eq("contact_id", record.contactId)
    .eq("recruiting_year", record.recruitingYear)
    .eq("is_potential_team", true)
    .maybeSingle();

  if (existingError) {
    console.error("Error checking existing potential team row", existingError);
    throw existingError;
  }

  if (existingPotential?.id) {
    throw new Error("This contact already has a Potential Teams row for this year.");
  }

  const insertPayload = buildCyclePayload({
  ...
  await logRecruitingActivity({
    ...
    actionDate: new Date().toISOString(),
    staffMember,
    summary: "Copied to Potential Teams",
  });
