...
async function loadTripNameMap(tripIds) {
  ...
  const { data, error } = await supabase
    .from("trips")
    .select("id, trip_name, name, location, created_at")
    .in("id", unique);
  ...
  for (const row of data || []) {
    map.set(row.id, {
      name: String(row.trip_name || row.name || "Untitled trip").trim(),
  ...
  for (const row of newTripRows) {
    events.push(
      makeEvent({
        ...
        title: "Team created",
        detail: [
          String(row.trip_name || row.name || "Untitled trip").trim(),
          row.location ? `Site: ${row.location}` : "",
        ]
          .filter(Boolean)
          .join(" · "),
        tripId: row.id,
        tripName: String(row.trip_name || row.name || "Untitled trip").trim(),
