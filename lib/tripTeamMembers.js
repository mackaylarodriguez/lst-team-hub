...
export async function listTripTeamMembers(tripId) {
  const { data, error } = await supabase
    ...
    .select("id, trip_id, first_name, last_name, email, start_date, end_date, fundraising_goal_amount, created_at, updated_at")
    .eq("trip_id", tripId)
...
export async function saveTripTeamMembers(tripId, members) {
  ...
  if (existingMembers.length > 0) {
    const { data, error } = await supabase
      .from("trip_team_members")
      .upsert(existingMembers, { onConflict: "id" })
      .select("id, trip_id, first_name, last_name, email, start_date, end_date, fundraising_goal_amount, created_at, updated_at");
  ...
  if (newMembers.length > 0) {
    const rowsToInsert = newMembers.map(({ id, ...member }) => member);
    const { data, error } = await supabase
      .from("trip_team_members")
      .insert(rowsToInsert)
      .select("id, trip_id, first_name, last_name, email, start_date, end_date, fundraising_goal_amount, created_at, updated_at");
