...
import { ROLE_ADMIN, ROLE_LEADER, ROLE_STAFF, ROLE_WORKER } from "@/lib/roles";
...
function memberHasOwnField(member, ...keys) {
  if (!member || typeof member !== "object") return false;
  return keys.some((key) => Object.prototype.hasOwnProperty.call(member, key));
}

function parseRosterFundraisingGoalAmount(value) {
  if (value === undefined || value === null || String(value).trim() === "") return null;
  const parsed = parseFloat(String(value).replace(/[^0-9.-]/g, ""), 10);
  return Number.isFinite(parsed) ? parsed : null;
}

/** Drop fundraising fields so a roster/role save cannot null out Neon links or amounts. */
export function omitTripMemberFundraisingFields(member) {
...
async function updateExistingTeamMemberRow(tripId, row) {
  const { id, ...payload } = row;
  if (!id) {
    throw new Error("Roster member id is required to update role and other fields.");
  }

  let attempt = payload;
  let omitTshirt = false;
  let selectColumns = TRIP_TEAM_MEMBER_SELECT_WITH_CELL_PHONE;
  let { data, error } = await supabase
    .from("trip_team_members")
    .update(attempt)
    .eq("id", id)
  ...
  if (error && isMissingCellPhoneColumnError(error)) {
    const { cell_phone, ...rest } = attempt;
    attempt = rest;
    selectColumns = TRIP_TEAM_MEMBER_SELECT_FULL;
    ({ data, error } = await supabase
      .from("trip_team_members")
      .update(attempt)
      .eq("id", id)
  ...
  if (error && isMissingTshirtSizeColumnError(error)) {
    omitTshirt = true;
    const { tshirt_size, ...rest } = attempt;
    attempt = rest;
    selectColumns = TRIP_TEAM_MEMBER_SELECT_NO_TSHIRT;
    ({ data, error } = await supabase
      .from("trip_team_members")
      .update(attempt)
      .eq("id", id)
  ...
  if (error && isMissingTravelsWithTeamColumnError(error)) {
    const { travels_with_team, ...rest } = attempt;
    attempt = rest;
    selectColumns = omitTshirt
      ? TRIP_TEAM_MEMBER_SELECT_NO_TRAVEL_NO_TSHIRT
      : TRIP_TEAM_MEMBER_SELECT_NO_TRAVEL;
    ({ data, error } = await supabase
      .from("trip_team_members")
  ...
  if (error && isMissingTeamRoleColumnError(error)) {
    throw new Error(
      "Could not save team role. The trip_team_members.team_role column is missing in the database."
    );
  }

  if (error) {
    console.error("Error updating trip team member", error);
    throw error;
  }
...
async function profileIsLeaderOnAnotherTrip(email, tripId) {
  const emailKey = normalizeEmail(email);
  if (!emailKey) return false;

  const { data, error } = await supabase
    .from("trip_team_members")
    .select("id, trip_id, team_role, email")
    .ilike("email", emailKey);

  if (error) {
    console.error("Error checking other trip leader roles", error);
    return true;
  }

  return (data || []).some((row) => {
    if (String(row.trip_id) === String(tripId)) return false;
    return normalizeRoleKey(row.team_role) === ROLE_LEADER;
  });
}

/** Keep hub `profiles.role` in sync when roster Worker/Leader changes. Never alter staff/admin. */
async function syncProfileRolesFromRoster(tripId, members) {
  for (const member of members || []) {
    const email = normalizeEmail(member?.email);
    if (!email) continue;

    const rosterRole = normalizeRoleKey(member?.team_role || member?.teamRole);
    ...
    if (error) {
      console.error("Unable to load profile for roster role sync", error);
      continue;
    }
    if (!profile?.id) continue;

    const currentRole = normalizeRoleKey(profile.role);
    if (currentRole === ROLE_ADMIN || currentRole === ROLE_STAFF) continue;

    let nextRole = null;
    if (rosterRole === ROLE_LEADER && currentRole === ROLE_WORKER) {
      nextRole = ROLE_LEADER;
    } else if (rosterRole === ROLE_WORKER && currentRole === ROLE_LEADER) {
      const stillLeader = await profileIsLeaderOnAnotherTrip(email, tripId);
      if (!stillLeader) nextRole = ROLE_WORKER;
    }

    if (!nextRole || nextRole === currentRole) continue;
    ...
    if (updateError) {
      console.error("Unable to sync profile role from roster", updateError);
    }
  }
}
...
export async function saveTripTeamMembers(tripId, members) {
  const normalizedMembers = (members || [])
    .map((member) => {
      ...
      // Only write these when the caller included them (e.g. trip create/edit form).
      // Roster/role saves omit them so existing Neon links and amounts stay intact.
      if (memberHasOwnField(member, "fundraisingUrl", "fundraising_url")) {
        payload.fundraising_url =
          String(member?.fundraisingUrl ?? member?.fundraising_url || "").trim() || null;
      }
      if (memberHasOwnField(member, "fundraisingGoalAmount", "fundraising_goal_amount")) {
        payload.fundraising_goal_amount = parseRosterFundraisingGoalAmount(
          member?.fundraisingGoalAmount ?? member?.fundraising_goal_amount
        );
      }
  ...
  if (existingMembers.length > 0) {
    ...
    for (const row of existingMembers) {
      updatedRows.push(await updateExistingTeamMemberRow(tripId, row));
    }
  ...
  await syncProfileRolesFromRoster(tripId, normalizedSaved);
