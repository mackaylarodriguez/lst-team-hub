...
export const TICKET_AGENCY_OPTIONS = [
  "LST",
  "Self Purchased",
  "High Point",
  "Reimbursement",
...
function isMissingTicketNotesColumnError(error) {
  const msg = String(error?.message || error?.details || error?.hint || "").toLowerCase();
  return (
    msg.includes("notes") &&
    (msg.includes("does not exist") || msg.includes("schema cache") || error?.code === "42703")
...
function omitTicketNotes(payload) {
  const next = { ...payload };
  delete next.notes;
  return next;
}
...
export async function saveTripTicket(ticket) {
  let payload = {
    ...
    notes: ticket.notes != null && String(ticket.notes).trim() !== "" ? String(ticket.notes) : null,
  ...
  if (error && isMissingTripTeamMemberIdColumnError(error)) {
    attemptPayload = omitTripTeamMemberId(attemptPayload);
    ({ data, error } = await upsert(attemptPayload));
  }
  if (error && isMissingTicketNotesColumnError(error)) {
    attemptPayload = omitTicketNotes(attemptPayload);
    ({ data, error } = await upsert(attemptPayload));
  }
...
export async function syncTripTicketsFromTeamMembers(trips = []) {
  async function fetchMemberPages(selectColumns) {
    const pageSize = 1000;
    const all = [];
    let from = 0;
    for (;;) {
  ...
  let { data: members, error: membersError } = await fetchMemberPages(
    "id, trip_id, first_name, last_name, email, team_role, travels_with_team"
  );

  if (membersError && isMissingTravelsWithTeamOnMemberError(membersError)) {
    ({ data: members, error: membersError } = await fetchMemberPages(
      "id, trip_id, first_name, last_name, email, team_role"
    ));
  }
  ...
  const inserts = (members || [])
    ...
    .map((member) => {
      ...
      const row = {
        ...
        intl_dom: defaultIntlDomForLocation(tripMeta.location),
