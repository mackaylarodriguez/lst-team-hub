...
function normalizeAckRow(row) {
  ...
  return {
    ...
    name:
      ...
      String(profile?.email || row.email || "").trim() ||
      "",
  };
}
...
export async function listTripTravelSafetyAcknowledgments(tripId) {
  ...
  const rows = (data || []).map(normalizeAckRow).filter(Boolean);
  ...
  if (profilesError) {
    console.warn("Unable to load profiles for travel safety acknowledgments", profilesError);
    return rows;
  }

  const profilesById = new Map((profiles || []).map((profile) => [String(profile.id), profile]));
  return rows.map((row) => {
    const profile = profilesById.get(String(row.userId));
