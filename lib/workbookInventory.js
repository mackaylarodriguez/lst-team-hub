...
/** Alternate spellings → canonical column keys used by the Sites workbook grid. */
const WORKBOOK_NAME_ALIASES = new Map([
  ["blue luke workbook", "blue luke"],
  ["blueluke", "blue luke"],
  ["beg1", "beg 1"],
  ["beg2", "beg 2"],
  ["beginner 1", "beg 1"],
...
/**
 * Normalize a saved workbook title to the grid column key when possible
 * (so "Reflection" and "REFLECTIONS" update the same count).
 */
export function resolveWorkbookInventoryKey(name) {
  const n = normalizeWorkbookNameKey(name);
  if (!n) return "";
  return WORKBOOK_NAME_ALIASES.get(n) || n;
}
...
export function mergeSiteWorkbookNotesWithDraft(existingRaw, columns, draftRecord) {
  ...
  for (const { name, qty } of parseAnyWorkbookInventoryString(String(existingRaw || ""))) {
    const k = resolveWorkbookInventoryKey(name);
