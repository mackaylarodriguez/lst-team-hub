...
function groupLeaderLabelValue(normalizedLabel) {
  ...
  if (!n) return null;
  const leaderCtx = n.includes("group leader") || n.includes("trip leader");
  if (n.includes("email") && leaderCtx) return "email";
  if (leaderCtx && (n.includes("cell") || n.includes("phone"))) return "cellPhone";
  if (leaderCtx && n.includes("name")) return "name";
  return null;
}
...
function fillGroupLeaderTripFields(sheet, range, headerRowIndex, trip) {
  ...
  for (let r = range.s.r; r <= lastDataRowAboveTable; r++) {
    for (let c = range.s.c; c < range.e.c; c++) {
      ...
      const existing = sheet[targetRef];
