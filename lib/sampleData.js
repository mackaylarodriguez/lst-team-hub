...
const TRIPS_STORAGE_KEY = "lstHubTrips";
...
export function getTrips() {
  const baseTrips = SAMPLE.trips.map(applyStaffTemplateIfMissing);
  ...
  try {
    const savedTrips = JSON.parse(localStorage.getItem(TRIPS_STORAGE_KEY) || "[]");
    return [...baseTrips, ...savedTrips.map(applyStaffTemplateIfMissing)];
  } catch {
    return baseTrips;
  }
}
...
export function addTrip(input) {
  const name = String(input?.name || "").trim();
  const location = String(input?.location || "").trim();
  const dates = String(input?.dates || "").trim();

  if (!name || !location || !dates) {
    throw new Error("Trip name, location, and dates are required.");
  }

  const trip = applyStaffTemplateIfMissing({
    id: createTripId(name),
    name,
    dates,
    location,
    staffLead: String(input?.staffLead || "").trim(),
    staffEmail: String(input?.staffEmail || "").trim(),
    participants: [],
    quickLinks: [],
    tasks: [],
    docs: [],
  ...
  const currentTrips = getTrips().filter(
    (existingTrip) => !SAMPLE.trips.some((baseTrip) => baseTrip.id === existingTrip.id)
  );
  const nextTrips = [...currentTrips, trip];
  localStorage.setItem(TRIPS_STORAGE_KEY, JSON.stringify(nextTrips));
  window.dispatchEvent(
    new CustomEvent(TRIPS_UPDATED_EVENT, {
      detail: { trip },
    })
  );
