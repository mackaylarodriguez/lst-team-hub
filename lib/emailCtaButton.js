...
export function buildEmailCtaFallbackUrl(url) {
  const link = String(url || "").trim();
  if (!link) return "";

  return `<table role="presentation" border="0" cellpadding="0" cellspacing="0" align="center" width="100%" style="margin:12px 0 0 0">
  <tr>
    <td align="center" style="padding:0 12px;font-size:12px;line-height:1.4;color:#71717a;word-break:break-all;font-family:Arial,Helvetica,sans-serif;">
      ${escapeHtml(link)}
    </td>
  </tr>
</table>`;
...
export function buildEmailCtaBlock({ href, label }) {
  const url = String(href || "").trim();
  if (!url) return "";
  return `${buildEmailCtaButton({ href: url, label })}${buildEmailCtaFallbackUrl(url)}`;
}
