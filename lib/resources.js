...
export async function addLinkResource({ title, link, workArea, tripId, category, resourceKey }) {
  try {
    const payload = {
...
export async function addPdfResource({ title, file, workArea, tripId, category, resourceKey }) {
...
export async function updateResource({ id, title, link, pdfUrl, workArea, category, resourceKey }) {
  try {
    const { data, error } = await supabase
      .from("trip_resources")
      .update({
